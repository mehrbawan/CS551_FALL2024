...
layout	(binding=0)	uniform	sampler2D	fogTex;  // corresponding to GL_TEXTURE0

uniform float Cnt; // for texture animation  
...
void	main(void)	{	
    ...
    vec4 Vg = Vsp - position; // view (distance) direction 
 	vec3 V = normalize(Vg.xyz); // 3D normalized view direction
    ...
    fColor = (Ie + Ia + (Id + I1d + I2d + I3d) + (Is + I1s + I2s + I3s))/2;      

 
 	vec2 tc; // fogTexture coordinates 
 	float Width = 1000, Height = 1000; 
 	tc = vec2(gl_FragCoord.x/Width, gl_FragCoord.y/Height);    // tc: s & t: 0-1
 	
 	// texture shifting
	tc.x = tc.x + Cnt/2000.0f;
	tc.y = tc.y + Cnt/3000.0f;
 
    vec4 fogColor = texture(fogTex, tc); 
 	//fColor = fogColor; 
 	
 	if (normal.x + normal.y == 0) fColor = fogColor; // for drawing the background
    // supposedly normal.z == 1, but it is a float
    //if (normal.z > 0.99f && normal.z < 1.01f) fColor = fogColor; 
 	

    // highest density: white, lowest density: black
 	float density = fogColor.x*fogColor.y*fogColor.z; 
 	float vDistance = length(Vg)/1000.0f;
 	float f = pow(2.71f, -density*vDistance); //e = 2.71f
	
    fColor = f*fColor + (1-f)*fogColor; 
