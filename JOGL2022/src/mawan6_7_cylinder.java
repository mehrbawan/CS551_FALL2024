...
public class mawan6_7_cylinder extends JOGL2_5_Cone {
	...
	void subdivideCylinder(float vPoints[], float v1[], float v2[], int depth) {
		float v0[] = { 0, 0, 0 };
		...
		if (depth == 0) {
			...
			/*
			 * gl.glBegin(GL.GL_POLYGON); // draw the side rectangles of the cylinder
			 * gl.glVertex3fv(v1,0); gl.glVertex3fv(v2,0); gl.glVertex3fv(v22,0);
			 * gl.glVertex3fv(v11,0); gl.glEnd();
			 */
	...
	public void drawCylinder() {
		int numofTriangle = 2 * 3 * 4 * (int) Math.pow(2, depth); // number of triangles after subdivision
