...
void	main(void)	{	
	...
	vec2 tc;
 	tc = vec2(gl_FragCoord.x/Width, gl_FragCoord.y/Height);    
 	 
     if (dCone==1) fColor = 0.3*fColor + 0.4*texture(vseTex, texCoord); // integrate color and texture
     else 
     if (dCylinder==1) fColor = texture(vseTex, texCoord); // integrate color and texture
     else 
     fColor = 0.6*fColor + 0.4*texture(gmuTex, tc); // integrate color and texture
